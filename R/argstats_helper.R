...
#'
#' @returns A data frame containing the calculated statistics, the relevant window, and relevant arg. The statistics include the time to the most recent common ancestor between populations/species (tmrca), estimates of the time to most recent common ancestor within populations/species (tmrcaw), the tmrca.50; the generations it takes for 50% of the haplotypes to coalesce, and the relative half life (rth); a normalized version of tmrca which is generated by dividing tmrca.50 by the tmrca (tmrca.50/tmrca). The output also indicates if populations/species are monophyletic and which population/species corresponds to which population.
...
argstats_helper <- function(arg.dat, pop1, pop2, pop1.name, pop2.name, n.haps = 2){
  ...
  ### Calculate the relative half life (rth)
  ...
  # Figure out when half of the haplotypes have coalesced
  tmrca.50.index <- ceiling(n.tips/2)

  # Get teh tmrca 50
  tmrca.50 <- coalescent.times[tmrca.50.index]

  rth <- tmrca.50/tmrca

  # Add tmrca.50 and rth
  arg.stats.df[i,5] <- tmrca.50
  arg.stats.df[i,6] <- rth
  ...
  if(ape::is.monophyletic(tree, tips= pop1)){
    ...
    arg.stats.df[i,9] <- pop1.dist.fix
    arg.stats.df[i,10] <- pop1.dist.fix
    arg.stats.df[i,11] <- TRUE
    ...
    if(nrow(pop1.df.filt) > 0){
      ...
      arg.stats.df[i,7] <- mean(tmrca_w)
      arg.stats.df[i,8] <- stats::median(tmrca_w)
      arg.stats.df[i,9] <- min(tmrca_w)
      arg.stats.df[i,10] <- max(tmrca_w)
      arg.stats.df[i,11] <- FALSE
  ...
  if(ape::is.monophyletic(tree, tips = pop2)){
    ...
    arg.stats.df[i,14] <- pop2.fix
    arg.stats.df[i,15] <- pop2.fix
    arg.stats.df[i,16] <- TRUE
    ...
    if(nrow(pop2.df.filt) > 0){
      ...
      arg.stats.df[i,12] <- mean(tmrca_w)
      arg.stats.df[i,13] <- stats::median(tmrca_w)
      arg.stats.df[i,14] <- min(tmrca_w)
      arg.stats.df[i,15] <- max(tmrca_w)
      arg.stats.df[i,16] <- FALSE
      ...
      arg.stats.df[i,14] <- NA
      arg.stats.df[i,15] <- NA
      arg.stats.df[i,16] <- FALSE
  ...
  remove(tmrca, tree, nwk, rth, tmrca.50, coalescent.times, n.tips, node.depths, tmrca.50.index)
