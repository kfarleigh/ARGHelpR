...
common ancestor between populations/species (tmrca), estimates of the
time to most recent common ancestor within populations/species (tmrcaw),
the tmrca.50; the generations it takes for 50% of the haplotypes to
coalesce, and the relative half life (rth); a normalized version of
tmrca which is generated by dividing tmrca.50 by the tmrca
(tmrca.50/tmrca). The output also indicates if populations/species are
monophyletic and which population/species corresponds to which
population.
