...
To help you understand the input format for ARGHelpR and how to convert
the output from ancestral recombination graph (ARG) estimation programs
into this format.

## Format expected by ARGHelpR

ARGHelpR uses bed files for analysis. There are four columns that are
tab-delimited. The first column contains the chromosome, the second
contains the start position, the third contains the end position, and
