...
An ancestral recombination graph (ARG) is a powerful phylogenetic
...
While traditional phylogenetic approaches often consider the genome a
single entity when inferring relationships between groups, ARGs account
for genetic recombination by allowing different genomic regions to have
distinct genealogical histories. This property of ARGs allows us to
disentangle intricate evolutionary forces such as selection and gene
flow that occur across the genome. Specifically, an ARG represents the
relationships among sampled haplotypes across the genome as a series of
local genealogies connected by recombination events. These genealogies
describe how haplotypes coalesce through time and we can use them to
estimate the time to the most recent common ancestor between lineages
(TMRCA_(B)), as well as the time to the most recent common ancestor
within a lineage (TMRCA_(W)).

ARGs have a variety of applications in population genetics, including
characterizing patterns of ancestry and genetic variation across the
genome. For example, ARGs can be used to investigate the distribution of
mutations and genetic variants, signatures of natural selection, and
patterns of introgression between populations or species ([Lewanski et
...
over time, we can use ARGs to estimate coalescence times of sampled
...
intermediate frequencies for long stretches of time would appear to have
older coalesence times than a neutral site ([Rasmussen et
...
population post divergence, reducing the coalescence time between
lineages, TMRCA_(B) ([Hubisz et
